...
const Dashboard = () => {
    ...
    const loadDashboardData = async () => {
        setLoading(true)
        try {
            const [myAuctions, joinedAuctions, myBids] = await Promise.all([
                ...
                <TabsContent value="my-bids" className="space-y-6">
                    ...
                    {data.myBids.length > 0 ? (
                        <div className="space-y-4">
                            {data.myBids.slice(0, 10).map((bid, index) => (
                                <BidItem key={index} bid={bid} />
                            ))}
                            {data.myBids.length > 10 && (
                                <div className="text-center pt-4">
                                    <p className="text-sm text-muted-foreground">
                                        Showing latest 10 bids out of {data.myBids.length} total bids
                                    </p>
                                </div>
                            )}
                        </div>
